...
///#/snippet commonAmdStart
	...
	function createEventHandler(handler, fThis, args, index, unprefixed, filterFunc) {
		...
		return function(event, triggerOriginalTarget) {
			...
			if (filterFunc(triggerOriginalTarget || event['target']) && (((stop = ((!handler.apply(fThis || triggerOriginalTarget || event['target'], args || [event, index])) || args) && unprefixed)) && !triggerOriginalTarget)) {
